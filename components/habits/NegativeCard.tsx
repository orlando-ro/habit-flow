...
interface NegativeCardProps {
  ...
  isCompleted: boolean; // Completed means failed
  onToggle: (e: React.MouseEvent) => void;
  streak: number;
}

export const NegativeCard = ({ habit, isCompleted, onToggle, streak }: NegativeCardProps) => {
      ...
      <div className="flex items-center justify-between relative z-10">
        <div>
          ...
          <p className="text-xs font-mono text-zinc-400 uppercase tracking-wider">
            {t('habits.clean')}: <span className="text-zinc-100">{streak}d</span>
          </p>
        </div>
        <motion.button
          whileTap={{ scale: 0.9 }}
